...
$destinations = mysql_query("SELECT COUNT(dst_ip) AS c1, COUNT(DISTINCT(src_ip)) AS c2, COUNT(DISTINCT(signature)) AS c3, 
                             INET_NTOA(dst_ip), map2.c_long as dst_c_long, MAX(timestamp) AS t
                             FROM event
...
echo "<table align=right cellpadding=0 cellspacing=0>
      <tr><td style=\"padding-right: 10px; font-size: 10px; font-weight:bold;\">Viewing: $i of $dstCount[0] destination IPs</td></tr></table><br><br>";
