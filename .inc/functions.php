...
function Status($status) {
    ...
    return "<td class=row style=\"background: $colour;\">$short</td>";
}

function SignatureLine($_sigName,$rC) {
    $sigName = htmlspecialchars(rtrim($_sigName));
    $rowID = "cm-sig-$rC";
    $html = "<td class=row name=$rowID id=$rowID>$sigName</td>";
    return $html;
}

function SigidLine($sigID,$rC) {
     $rowID = "cm-sid-$rC";
     $html = "<td class=row name=$rowID id=$rowID>$sigID</td>";
     return $html;
}
...
function IPLine($ip,$port,$type,$cc,$rC) {
    ...
    if ($port == 'NA') {
        ...
        if ($port == '') {
            $phtml = "<td class=row name=xxx id=xxx>--</td>";
        } else {
            $phtml = "<td class=row name=$portID id=$portID>$port</td>";
        }
    }
    ...
    if ($cc == '42') {
        ...
        if (rfc1918($ip) == '0') {
            $cchtml = "<td class=row name=$ccID id=$ccID style=\"padding: none; color: gray;\">$cc</td>";
        } else {
            $cchtml = "<td class=row name=$ccID id=$ccID style=\"font-weight: bold;\">$cc</td>";
        }
    }

    $ipInt = sprintf("%u", ip2long($ip));

    $html = "<td sorttable_customkey=\"$ipInt\" class=row name=$rowID id=$rowID>$ip</td>";
    $html.= "$phtml";
    $html.= "$cchtml";
